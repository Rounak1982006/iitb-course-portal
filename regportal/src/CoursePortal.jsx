...
const INITIAL_COURSES = [
  {
    ...
    desc: "Foundations of programming, algorithms, and computational thinking. Covers Python, data structures, recursion, and problem-solving methodologies.",
  },
  {
    id: 2,
    code: "ENT101",
    ...
    seats: 60,
    level: "All Levels",
    instructor: "Prof. Rakesh Basant",
    dept: "Shailesh J. Mehta School of Management",
    desc: "Principles of entrepreneurship, startup ideation, business model canvas, funding strategies, and real-world venture case studies from IIT Bombay alumni.",
  },
  {
    id: 3,
    code: "MA110",
    ...
    duration: "16 weeks",
    schedule: "Mon/Tue/Thu/Fri, 8–9 AM",
    seats: 200,
    level: "Beginner",
    instructor: "Prof. A. K. Lerner",
    dept: "Department of Mathematics",
    desc: "Differential and integral calculus, sequences and series, multivariable calculus, matrices, eigenvalues, and vector spaces for engineering applications.",
  },
  {
    id: 4,
    code: "ME104",
    ...
    duration: "16 weeks",
    schedule: "Mon/Wed/Fri, 11 AM–12 PM",
    seats: 150,
    level: "Intermediate",
    instructor: "Prof. Salil Kulkarni",
    dept: "Department of Mechanical Engineering",
    desc: "Statics, dynamics, kinematics of particles and rigid bodies, Newton's laws, work-energy theorem, and impulse-momentum applied to engineering systems.",
  },
  {
    id: 5,
    code: "PH117",
    ...
    desc: "Hands-on experiments in mechanics, optics, electromagnetism, and modern physics. Emphasis on error analysis, scientific writing, and lab report preparation.",
  },
  {
    id: 6,
    code: "SOM101",
    ...
    seats: 50,
    level: "All Levels",
    instructor: "Prof. Bharat Rao",
    dept: "Shailesh J. Mehta School of Management",
    desc: "Introduction to organisational behaviour, operations management, marketing principles, financial accounting, and strategic management for engineers.",
  },
];

const INITIAL_USERS = [
  { id: "admin1", email: "admin@iitb.ac.in", password: "admin123", name: "Prof. Admin", role: "admin" },
  { id: "u1", email: "25B0001@iitb.ac.in", password: "iitb1234", name: "Aryan Sharma", role: "user" },
  { id: "u2", email: "25B0002@iitb.ac.in", password: "iitb1234", name: "Sneha Patel", role: "user" },
];
...
let _regId = 200;
let _courseId = 20;
const newRegId = () => String(++_regId);
const newCourseId = () => ++_courseId;

const emptyCourse = () => ({
  code: "", title: "", category: "Computer Science", duration: "",
  schedule: "", seats: "",
  level: "Beginner", instructor: "", dept: "", desc: "",
});
...
export default function App() {
  ...
  const handleLogin = () => {
    setAuthErr("");
    const found = allUsers.find(u => u.email === loginForm.email && u.password === loginForm.password);
    if (!found) { setAuthErr("Invalid email or password."); return; }
    setUser(found);
    setView(found.role === "admin" ? "admin" : "portal");
    showToast(`Welcome, ${found.name.split(" ")[0]}!`);
  };

  const handleSignup = () => {
    setAuthErr("");
    if (!signupForm.name || !signupForm.email || !signupForm.password) { setAuthErr("Please fill all fields."); return; }
    if (signupForm.password !== signupForm.confirm) { setAuthErr("Passwords do not match."); return; }
    if (allUsers.find(u => u.email === signupForm.email)) { setAuthErr("Email already registered."); return; }
    const nu = { id: "u" + Date.now(), email: signupForm.email, password: signupForm.password, name: signupForm.name, role: "user" };
    setAllUsers(p => [...p, nu]);
    setUser(nu);
    setView("portal");
    showToast(`Welcome, ${nu.name.split(" ")[0]}!`);
  };
  ...
  const myRegForCourse = (courseId) => myRegistrations.find(r => r.courseId === courseId);
  ...
  const submitReg = () => {
    const reg = {
      id: newRegId(), userId: user.id, courseId: regModal.id,
      userName: user.name, userEmail: user.email,
      courseTitle: regModal.title, courseCode: regModal.code,
      phone: regForm.phone, rollno: regForm.rollno, goal: regForm.goal,
      appliedAt: new Date().toLocaleDateString("en-IN"), status: "pending",
    };
  ...
  const openEditCourse = (c) => { setCourseForm({ ...c, seats: String(c.seats) }); setCourseModal({ mode: "edit", id: c.id }); };
  ...
  const saveCourse = () => {
    ...
    if (courseModal.mode === "add") {
      setCourses(p => [...p, { ...courseForm, id: newCourseId(), seats: Number(courseForm.seats) || 50 }]);
      showToast("Course added!");
    } else {
      setCourses(p => p.map(c => c.id === courseModal.id ? { ...courseForm, id: c.id, seats: Number(courseForm.seats) || c.seats } : c));
  ...
  const deleteCourse = (id) => {
    ...
    setRegistrations(p => p.filter(r => r.courseId !== id));
    showToast("Course deleted.");
  };

  // ── ADMIN: REGISTRATIONS ──────────────────────────────────────────────────
  const updateRegStatus = (id, status) => {
  ...
  const filteredCourses = courses.filter(c => {
    ...
    const matchQ = c.title.toLowerCase().includes(searchQ.toLowerCase()) ||
  ...
  const S = {
    ...
    heroH1: { fontSize: 24, fontWeight: 800, color: "#ffffff", margin: "0 0 4px", letterSpacing: "-0.5px" },
      ...
      <div style={{ ...S.root, background: "#f8f9fc" }}>
        ...
        <div style={S.main}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 10, marginBottom: "1.5rem" }}>
            {[
              { label: "Total Courses", value: courses.length, color: "#042045" },
              { label: "Applications", value: registrations.length, color: "#1565C0" },
              { label: "Pending Review", value: pendingCount, color: "#E65100" },
              { label: "Students", value: allUsers.filter(u => u.role === "user").length, color: "#2E7D32" },
            ].map(s => (
          ...
          {adminTab === "registrations" && (
            <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e8e8f0", overflow: "hidden" }}>
              <div style={{ padding: "1rem 1.25rem", display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f3f4f6", flexWrap: "wrap", gap: 8 }}>
                <span style={{ fontWeight: 700, fontSize: 14 }}>All Applications ({registrations.length})</span>
                    ...
                    <tbody>
                      {filteredRegs.map(r => (
                        <tr key={r.id} className="hrow">
                          ...
                          <td style={S.td}>{r.rollno || "—"}</td>
                          <td style={S.td}><span style={{ fontWeight: 800, color: "#042045" }}>{r.courseCode}</span> — {r.courseTitle}</td>
                          <td style={S.td}>{r.appliedAt}</td>
          ...
          {adminTab === "users" && (
            <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e8e8f0", overflow: "hidden" }}>
              ...
              <div style={{ overflowX: "auto" }}>
                <table style={S.table}>
                  <thead><tr>{["Name", "Email", "Role", "Courses Applied"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr></thead>
                  <tbody>
                    {allUsers.map(u => {
                      ...
                      return (
                        <tr key={u.id} className="hrow">
                          ...
                          <td style={S.td}>
                            <span style={{ padding: "2px 8px", borderRadius: 5, fontSize: 10, fontWeight: 700, background: u.role === "admin" ? "#E3F2FD" : "#F3E8FF", color: u.role === "admin" ? "#1565C0" : "#6B21A8" }}>
                              {u.role.toUpperCase()}
                            </span>
                          </td>
                          <td style={S.td}>
                            {userRegs.length > 0
                              ? userRegs.map(r => <span key={r.id} style={{ ...S.statusPill(r.status), marginRight: 4 }}>{r.courseCode}</span>)
    ...
    <div style={S.root}>
      ...
      <main style={S.main}>
        <div style={{ marginBottom: "1.25rem" }}>
          <h1 style={S.heroH1}>Browse & Enroll in Courses</h1>
          <p style={S.heroPara}>{filteredCourses.length} course{filteredCourses.length !== 1 ? "s" : ""} available · Semester Registration Open</p>
        </div>

        {/* My Applications Banner */}
        {myRegistrations.length > 0 && (
          <div style={{ background: "#fff", borderRadius: 12, border: "1px solid #e8e8f0", padding: "0.9rem 1.25rem", marginBottom: "1.1rem" }}>
            <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 7, color: "#042045" }}>My Applications</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 7 }}>
              {myRegistrations.map(r => (
                <div key={r.id} style={{ display: "flex", alignItems: "center", gap: 5, background: "#f9fafb", border: "1px solid #e5e7eb", borderRadius: 8, padding: "4px 10px" }}>
                  <span style={{ fontWeight: 800, fontSize: 11, color: "#042045" }}>{r.courseCode}</span>
                  <span style={{ fontSize: 11, color: "#6b7280" }}>{r.courseTitle}</span>
                  <span style={S.statusPill(r.status)}>{STATUS_STYLES[r.status]?.label}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={S.filterBar}>
          {categories.map(cat => (
            <button key={cat} style={S.chip(filterCat === cat)} onClick={() => setFilterCat(cat)}>{cat}</button>
          ))}
          <div style={S.searchWrap}>
            <span style={S.searchIcon}>🔍</span>
            <input style={S.searchInp} placeholder="Code, title, instructor..."
      ...
      {detailCourse && (
        <div style={S.overlay} onClick={e => e.target === e.currentTarget && setDetailCourse(null)}>
          <div style={S.modal}>
            ...
            <p style={{ fontSize: 13, color: "#374151", lineHeight: 1.7, marginBottom: "1rem" }}>{detailCourse.desc}</p>
            {[["Instructor", detailCourse.instructor], ["Schedule", detailCourse.schedule], ["Duration", detailCourse.duration], ["Seat Capacity", `${detailCourse.seats} students`]].map(([l, v]) => (
            ...
            {regStep === 1 && (
              <>
                ...
                <div style={{ background: "#f9fafb", borderRadius: 10, padding: "0.85rem", marginBottom: "1.1rem", border: "1px solid #e5e7eb" }}>
                  {[["Instructor", regModal.instructor], ["Schedule", regModal.schedule], ["Duration", regModal.duration], ["Seats", `${regModal.seats} available`]].map(([l, v]) => (
